...
struct priv {
    ...
    // Bridge specific
    struct mp_image_params m_params;
    struct voctrl_performance_data *perf_data;
    struct vo *vo;
    pl_tex planes[4];
    int dxgi_colorspace; // DXGI color space from app (-1 = unknown)
};
...
static const struct m_sub_options gl_next_conf = {
...
static void update_hook_opts_dynamic(struct priv *p, const struct pl_hook *hook, const struct mp_image *mpi);
static void update_hook_opts(struct priv *p, char **opts, const char *shaderpath, const struct pl_hook *hook);
...
static struct mp_image *get_image(struct render_backend *ctx, int imgfmt, int w, int h,
                      ...
                      struct pl_frame *frame)
{
    ...
    struct pl_color_space frame_color = mpi->params.color;
    struct pl_color_repr frame_repr = mpi->params.repr;
...
static void update_options(struct priv *p)
{
    ...
    /* next_opts is allocated directly, no cache to update */
    if (changed)
                                 ...
                                 float min_luma, bool hint)
{
    ...
    if (dither_depth == 0) {
        ...
        if (sw && sw->fns->color_depth && sw->fns->color_depth(sw) != -1) {
...
/* Convert DXGI color space type to pl_color_space.
 * Only sets primaries and transfer function - leaves hdr metadata (max_luma etc)
...
static struct pl_color_space dxgi_to_pl_colorspace(int dxgi)
{
    ...
    /* 2. Fallback to libmpv definitions if they don't clash */
    switch (dxgi) {
    case MPV_DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020:
    case MPV_DXGI_COLOR_SPACE_RGB_STUDIO_G2084_NONE_P2020:
    case MPV_DXGI_COLOR_SPACE_YCBCR_STUDIO_G2084_LEFT_P2020:
    ...
    case MPV_DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709:
    case MPV_DXGI_COLOR_SPACE_RGB_STUDIO_G22_NONE_P709:
    case MPV_DXGI_COLOR_SPACE_YCBCR_FULL_G22_NONE_P709_X601:
    case MPV_DXGI_COLOR_SPACE_YCBCR_STUDIO_G22_LEFT_P601:
...
static void update_ra_ctx_options(struct priv *p, struct ra_ctx_opts *ctx_opts)
{
...
static int init(struct render_backend *ctx, mpv_render_param *params)
{
    ...
    struct gl_video_opts *vopts = p->opts_cache->opts;
    vopts->deband_opts = talloc_zero(vopts, struct deband_opts);
    *vopts->deband_opts = (struct deband_opts) {
        .iterations = 1, .threshold = 48.0, .range = 16.0, .grain = 32.0
    };
    vopts->icc_opts = talloc_zero(vopts, struct mp_icc_opts);
    *vopts->icc_opts = (struct mp_icc_opts) {
        .size_str = (char *)"auto", .intent = 0, .use_embedded = true, .cache = true
    };
    ...
    *p->next_opts = (struct gl_next_opts) {
    ...
    /* Read DXGI colorspace from init params if provided */
    int *csp_param = get_mpv_render_param(params, MPV_RENDER_PARAM_DXGI_COLORSPACE, NULL);
    if (csp_param)
        p->dxgi_colorspace = *csp_param;
    ...
    for (int n = 0; context_backends[n]; n++) {
        if (strcmp(context_backends[n]->api_name, api) == 0) {
            p->context = talloc_zero(p, struct libmpv_gpu_context);
            if (!ctx->hwdec_devs) {
                ctx->hwdec_devs = hwdec_devices_create();
            }
            *p->context = (struct libmpv_gpu_context){
                .global = ctx->global, .log = ctx->log, .hwdec_devs = ctx->hwdec_devs, .fns = context_backends[n]
            };
            break;
        }
    }
    ...
    if (p->context->fns->init(p->context, params) < 0) {
        return MPV_ERROR_NOT_IMPLEMENTED;
    }
    ...
    struct gl_video_opts *gl_opts = p->opts_cache->opts;
    ra_hwdec_ctx_init(&p->hwdec_ctx, ctx->hwdec_devs, gl_opts->hwdec_interop, false);

    mp_mutex_init(&p->dr_lock);

    if (gl_opts->shader_cache)
        cache_init(p, &p->shader_cache, 10 << 20, gl_opts->shader_cache_dir);
    if (gl_opts->icc_opts && gl_opts->icc_opts->cache)
...
{
    struct priv *p = ctx->priv;
    if (!p || !p->gpu) return false;
    if (imgfmt == IMGFMT_D3D11) return true;
    if (ra_hwdec_get(&p->hwdec_ctx, imgfmt))
        return true;
    bool supported = format_supported(p, imgfmt, false);
    if (!supported)
        supported = format_supported(p, imgfmt, true);
    return supported;
}
...
static int render(struct render_backend *ctx, mpv_render_param *params, struct vo_frame *frame)
{
    ...
    if (p->perf_data)
        p->perf_data->fresh.count = 0;

    struct pl_render_params render_params = pars->params;
    const struct gl_video_opts *opts = p->opts_cache->opts;
    ...
    render_params.preserve_mixing_cache = p->next_opts->inter_preserve && !frame->still;
    if (frame->still)
    ...
    if (frame->current && frame->current->params.vflip) {
        ...
        render_params.distort_params = &pars->distort_params;
    } else {
    ...
    /* First try to get target_csp from real swapchain (if available) */
    if (p->ra_ctx->swapchain && p->ra_ctx->swapchain->fns->target_csp)
        target_csp = p->ra_ctx->swapchain->fns->target_csp(p->ra_ctx->swapchain);
    /* Fallback: use DXGI colorspace passed by app during init */
    if (target_csp.transfer == PL_COLOR_TRC_UNKNOWN && p->dxgi_colorspace >= 0) {
        target_csp = dxgi_to_pl_colorspace(p->dxgi_colorspace);
    }
    
    // Throttled log: Only log target colorspace once per session or on change
    ...
    /* target_hint requires a real swapchain (p->sw). In bridge mode we don't have
     * one - the app handles compositor signaling via SetColorSpace1 directly.
     * Tone mapping still works correctly via dxgi_to_pl_colorspace(). */
    bool target_hint = p->sw && (p->next_opts->target_hint == 1 ||
    ...
    if (target_hint && frame->current) {
        ...
        const struct pl_color_space *target_cs = &target_csp;
        hint = *source;
        if (!hint.hdr.min_luma)
            hint.hdr.min_luma = target_cs->hdr.min_luma;
        if (p->next_opts->target_hint_mode == 0) {
            ...
            if (target_cs->hdr.max_luma) {
                hint.hdr.max_luma = target_cs->hdr.max_luma;
                hint.hdr.min_luma = target_cs->hdr.min_luma;
                hint.hdr.max_cll  = target_cs->hdr.max_cll;
                hint.hdr.max_fall = target_cs->hdr.max_fall;
            }
        }
        ...
        if (pl_color_transfer_is_hdr(target_cs->transfer) && opts->tone_map.inverse) {
            hint.transfer     = target_cs->transfer;
            hint.hdr.max_luma = target_cs->hdr.max_luma;
            hint.hdr.min_luma = target_cs->hdr.min_luma;
            hint.hdr.max_cll  = target_cs->hdr.max_cll;
            hint.hdr.max_fall = target_cs->hdr.max_fall;
    ...
    if (target.color.transfer == PL_COLOR_TRC_UNKNOWN || target.color.primaries == PL_COLOR_PRIM_UNKNOWN)
        pl_color_space_merge(&target.color, &target_csp);
            ...
            if (opts->blend_subs) {
                ...
                if (fp->osd_sync < p->osd_sync) {
                    ...
                    struct mp_osd_res res = {
                        ...
                        .h = h,
                        .ml = -image->crop.x0 * rx,
                        .mr = (image->crop.x1 - frame->current->w) * rx,
                        .mt = -image->crop.y0 * ry,
                        .mb = (image->crop.y1 - frame->current->h) * ry,
                        .display_par = 1.0,
                    };
    ...
    pl_frames_infer_mix(p->rr, &mix, &target, &(struct pl_frame){0});
...
static void screenshot(struct render_backend *ctx, struct vo_frame *frame, struct voctrl_screenshot *args)
{
    struct priv *p = ctx->priv;
    pl_options pars = p->pars;
    pl_gpu gpu = p->gpu;
    ...
    update_options(p);
    struct pl_render_params render_params = pars->params;
    render_params.info_callback = NULL;
    render_params.skip_caching_single_frame = true;
    render_params.preserve_mixing_cache = false;
    render_params.frame_mixer = NULL;

    struct pl_peak_detect_params peak_params;
    if (render_params.peak_detect_params) {
        peak_params = *render_params.peak_detect_params;
    ...
    if (!pl_render_image(p->rr, &image, &target, &render_params)) {
        MP_ERR(p, "Failed rendering frame!\n");
        goto done;
    }
