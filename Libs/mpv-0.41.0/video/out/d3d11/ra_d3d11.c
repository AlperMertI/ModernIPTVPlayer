...
{
    struct ra_d3d11 *p = ra->priv;
    MP_INFO(ra, "[NATIVE_D3D] RA destroy START\n");
    ...
    if (p->ctx) {
        MP_INFO(ra, "[NATIVE_D3D] Clearing and flushing context...\n");
        ...
        MP_INFO(ra, "[NATIVE_D3D] Context flushed\n");
    }
    SAFE_RELEASE(p->ctx1);
    SAFE_RELEASE(p->ctx);
    ...
    MP_INFO(ra, "[NATIVE_D3D] RA destroy END\n");
    // talloc_free(ra) is handled by the caller (ra_free) in core
}
