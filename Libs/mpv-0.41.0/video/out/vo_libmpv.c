...
void mpv_render_context_free(mpv_render_context *ctx)
{
    ...
    MP_INFO(ctx, "[NATIVE_RC] mpv_render_context_free START\n");
    ...
    MP_INFO(ctx, "[NATIVE_RC] Render context detached from core\n");

    if (atomic_load(&ctx->in_use)) {
        MP_INFO(ctx, "[NATIVE_RC] Context in use, killing video...\n");
        ...
        MP_INFO(ctx, "[NATIVE_RC] Context no longer in use\n");
    }
    ...
    if (ctx->renderer) {
        MP_INFO(ctx, "[NATIVE_RC] Destroying renderer...\n");
        ctx->renderer->fns->destroy(ctx->renderer);
        MP_INFO(ctx, "[NATIVE_RC] Renderer destroyed\n");
        // NOTE: ra_free (which calls this) or the caller (mpv_render_context_free) is responsible for talloc_free(ctx->renderer)
    }
    talloc_free(ctx->dr);
    ...
    MP_INFO(ctx, "[NATIVE_RC] mpv_render_context_free END\n");
    talloc_free(ctx);
}
