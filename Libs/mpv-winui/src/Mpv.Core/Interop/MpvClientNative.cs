...
public partial class MpvClientNative
{
    ...
    private static int _observeTotalCount = 0;
    private static int _unobserveTotalCount = 0;
    public void ObserveProperty(string name, MpvFormat format, ulong requestId = 0)
    {
        Debug.WriteLine($"[LEAK_DEBUG] ObserveProperty #{++_observeTotalCount}: {name} (format={format}, req={requestId})");
        mpv_observe_property(Handle, requestId, name, format);
    }

    public void UnObserveProperties(ulong requestId = 0)
    {
        Debug.WriteLine($"[LEAK_DEBUG] UnObserveProperties #{++_unobserveTotalCount} (req={requestId})");
    ...
    public MpvEvent WaitEvent(double timeout = -1)
    {
